...
class MemberController extends Controller
{
    ...
    public function create(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'member_pic'     => 'nullable|image|mimes:jpeg,png,jpg,webp|max:5120',
            'member_pic_cropped' => 'nullable|string', // base64 from Cropper
        ]);
        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                $imagePath = $this->saveCroppedOrUploaded(
                    $request->input('member_pic_cropped'),
                    $request->file('member_pic'),
                    'uploads/member'
                );
    ...
    public function update($member_id, Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'register_date'  => 'required|date',
            'member_pic'     => 'nullable|image|mimes:jpeg,png,jpg,webp|max:5120',
            'member_pic_cropped' => 'nullable|string', // base64
        ]);

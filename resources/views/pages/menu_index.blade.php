...
<style>
    ...
    /* Pagination */
    .pagination { display: inline-flex; gap: 5px; padding-left: 0; list-style: none; justify-content: center; }
    .pagination li a, .pagination li span {
        display: inline-flex; justify-content: center; align-items: center;
        min-width: 40px; height: 40px; border-radius: 50%; text-decoration: none;
        color: #333; background: #f0f0f0; border: none; font-weight: 500;
        padding: 0 12px;
    }
    .pagination li a:hover { background: #7f5aff; color: #fff; }
    .pagination li.active span { background: #7f5aff; color: #fff; }
...
<div class="row mb-4">
    <div class="col-12">
        <form action="{{ url()->current() }}" method="GET" class="d-flex flex-wrap filter-wrap justify-content-center" id="filterForm">
            ...
            {{-- Buttons --}}
            <button type="submit" class="btn" style="background-color: #6b4226; color: #fff8da">Search</button>
            @if($hasFilter)
...
@section('js_before')
{{-- Removed the auto-typing submit script so only Category autosubmits; Search uses the button --}}
@endsection

...
<table class="table table-bordered table-striped table-hover">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td align="center">
                <a href="/member/{{ $row->member_id }}" class="btn btn-warning btn-sm" style="color: #000">Edit</a>
            </td>
            <td align="center">
                <button type="button" class="btn btn-danger btn-sm"
                    onclick="deleteConfirm({{ $row->member_id }})">Delete</button>
                <form id="delete-form-{{ $row->member_id }}" action="/member/remove/{{ $row->member_id }}" method="POST"
                    style="display: none;">
                    @csrf
                    @method('delete')
                </form>
    ...
    function deleteConfirm(member_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById('delete-form-' + member_id).submit();
            }
        })
    }
</script>